...
describe("Custodian single LP", async function () {
    ...
    it("Should allow deposits and correctly account for LP", async function () {
        ...
        expect(await custodian.calculateLpToUsdc(lpBalance)).to.equal(1000000000);
    });

    it("Should correctly account for LP shares", async function () {
        const lpBalance = await token.balanceOf(alice.address);
        const usdcBalance = await custodian.calculateLpToUsdc(lpBalance);
        ...
        expect(await custodian.getTotalBalance()).to.equal(await custodian.calculateLpToUsdc(lpBalance));
    });
});
...
describe("Custodian multiple LP", async function () {
    ...
    it("3 deposits, 1 withdrawal", async function () {
        ...
        expect(await custodian.calculateLpToUsdc(aliceLp)).to.equal(6000000000);
        expect(await custodian.calculateLpToUsdc(bobLp)).to.equal(6000000000);
        expect(await custodian.calculateLpToUsdc(cathyLp)).to.equal(3000000000);
    });
